  ...
  <section class="rounded-3xl border border-base-300 bg-base-100/80 p-5 shadow-sm backdrop-blur">
    <div class="flex flex-wrap items-end justify-between gap-3">
      <div>
        ...
        <p class="mt-1 text-sm text-base-content/70">เลือกเด็ก 1 คน แล้วดูว่า “ลงทะเบียนวิชาอะไรไปบ้าง”</p>
      </div>
      <a class="rounded-2xl border border-base-300 bg-base-100 px-4 py-2.5 text-sm hover:bg-base-200" href="/tracks/?route=register_track&year_id=<?= (int)$yearId ?><?= $selectedCode !== '' ? '&q=' . e(rawurlencode($selectedCode)) : '' ?>">🧾 ไปหน้าลงทะเบียน</a>
    </div>
    ...
    <form class="mt-4 grid gap-3 rounded-3xl border border-base-300 bg-base-200 p-4 md:grid-cols-6" method="get" action="/tracks/">
      ...
      <div class="md:col-span-6 flex flex-wrap items-center gap-2">
        ...
        <a class="rounded-2xl border border-base-300 bg-base-100 px-4 py-2.5 text-sm hover:bg-base-200" href="/tracks/?route=student_track">🧹 ล้างตัวกรอง</a>
      </div>
    </form>
        ...
        <?php else: ?>
          <div class="px-4 py-4">
            <div class="flex flex-wrap items-start justify-between gap-3">
              ...
              <div class="flex flex-wrap items-center gap-2">
                <a class="rounded-2xl border border-base-300 bg-base-100 px-3 py-2 text-xs hover:bg-base-200" href="/tracks/?route=register_track&year_id=<?= (int)$yearId ?>&q=<?= e(rawurlencode((string)$student['student_code'])) ?>">🧾 ไปแก้ไข/ลงทะเบียนเพิ่ม</a>
                <a class="rounded-2xl bg-neutral px-3 py-2 text-xs font-medium text-white hover:opacity-95" href="/tracks/?route=report_statement&year_id=<?= (int)$yearId ?>&student_code=<?= e(rawurlencode((string)$student['student_code'])) ?>#preview">🖨️ ใบ Statement</a>
              </div>
            </div>
          </div>
            ...
            <table class="min-w-full bg-base-100 text-sm">
              ...
              <tbody class="divide-y divide-base-300">
                  ...
                  <tr class="hover:bg-base-200">
                    <td class="px-4 py-3">
                      ...
                      <?php if (!empty($r['subject_description'])): ?>
                        <div class="mt-1 text-xs text-base-content/60"><?= e((string)$r['subject_description']) ?></div>
                      <?php endif; ?>
                    </td>
                    <td class="px-4 py-3 text-xs text-base-content/60"><?= e((string)$r['created_at']) ?></td>
                    <td class="px-4 py-3">
                ...
                <?php if (empty($studentRegs)): ?>
                  <tr>
                    <td colspan="3" class="px-4 py-10 text-center text-sm text-base-content/70">ยังไม่มีรายการลงทะเบียน</td>
                  </tr>
                <?php endif; ?>
              </tbody>
            </table>
