...
<div class="mx-auto max-w-4xl">
  <div class="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
    <div>
      ...
      <p class="mt-1 text-sm text-base-content/70">สำหรับผู้ดูแลระบบ: ดาวน์โหลดไฟล์ backup และกู้คืนข้อมูลจากไฟล์ .sql (เฉพาะไฟล์ที่สร้างจากระบบ)</p>
    </div>
  </div>
  ...
  <div class="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-2">
    ...
    <section class="rounded-3xl border border-base-300 bg-base-100/70 p-5 shadow-sm">
      <h2 class="text-lg font-semibold">💾 Backup (ดาวน์โหลด)</h2>
      <p class="mt-1 text-sm text-base-content/70">กดเพื่อสร้างไฟล์ .sql และบันทึกไว้ที่เซิร์ฟเวอร์ จากนั้นสามารถกดดาวน์โหลดได้</p>

      <form class="mt-4" method="post" action="/tracks/backup_restore" autocomplete="off">
        <input type="hidden" name="_csrf" value="<?= e($csrf) ?>" />
        <input type="hidden" name="action" value="create_backup" />
        <button type="submit" class="inline-flex items-center justify-center rounded-2xl bg-neutral px-4 py-2 text-sm font-semibold text-white hover:bg-black">
      ...
      <div class="mt-4">
        ...
        <?php if (empty($backups)): ?>
          <div class="mt-2 text-sm text-base-content/70">ยังไม่มีไฟล์ backup</div>
        <?php else: ?>
          <div class="mt-2 overflow-hidden rounded-2xl border border-base-300 bg-base-100">
            <table class="w-full text-sm">
              <thead class="bg-base-200/70 text-base-content/80">
                <tr>
              ...
              <tbody class="divide-y divide-base-300">
                <?php foreach ($backups as $b): ?>
                  ...
                  <tr>
                    ...
                    <td class="px-3 py-2 text-base-content/70"><?= $mtime > 0 ? e(date('Y-m-d H:i', $mtime)) : '-' ?></td>
                    <td class="px-3 py-2 text-right text-base-content/70"><?= e(number_format(max(0, (int)round($size / 1024)))) ?> KB</td>
                    <td class="px-3 py-2">
                      <div class="flex flex-wrap justify-end gap-2">
                        <a class="rounded-xl border border-base-300 bg-base-100 px-3 py-1.5 text-xs hover:bg-base-200" href="<?= e($downloadUrl) ?>">⬇️ ดาวน์โหลด</a>
    ...
    <section class="rounded-3xl border border-base-300 bg-base-100/70 p-5 shadow-sm">
      ...
      <p class="mt-1 text-sm text-base-content/70">อัปโหลดไฟล์ .sql ที่ดาวน์โหลดจากระบบนี้เพื่อกู้คืนข้อมูล (ฐาน sukhon_track เท่านั้น)</p>
      ...
      <form class="mt-4 space-y-3" method="post" action="/tracks/backup_restore" enctype="multipart/form-data" data-confirm="ยืนยันการกู้คืนข้อมูลจากไฟล์ที่อัปโหลด? ข้อมูลเดิมอาจถูกเขียนทับ">
        ...
        <label class="block text-sm">
          <span class="text-base-content">ไฟล์ .sql</span>
          <input type="file" name="sql_file" accept=".sql,text/plain,application/sql" class="mt-1 block w-full rounded-2xl border border-base-300 bg-base-100 px-3 py-2 text-sm" />
        </label>
        ...
        <div class="rounded-2xl border border-base-300 bg-base-200/60 px-3 py-2 text-xs text-base-content/70">
